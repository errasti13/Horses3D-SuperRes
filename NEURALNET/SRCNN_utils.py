...
def create_srgan_model(input_shape, num_layers):
    ...
    discriminator.add(Conv3D(64, kernel_size=3, strides=(1, 1, 1), padding='same'))
    discriminator.add(LeakyReLU(alpha=0.2))

    discriminator.add(Conv3D(64, kernel_size=3, strides=(2, 2, 2), padding='same'))
    ...
    discriminator.compile(loss='binary_crossentropy', optimizer=Adam(learning_rate=2e-4), metrics=['accuracy'])
